...
const Home: React.FC<HomeProps> = ({ products, setLocation, location }) => {
  ...
  const categories = [
    ...
    { name: Category.BOOKS, icon: '📚', color: 'bg-green-50 text-green-600' },
  ...
  return (
    <div className="space-y-12 pb-20">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-green-600 to-emerald-700 text-white py-24 px-4 overflow-hidden relative">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 relative z-10">
          <div className="flex-1 space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/marketplace" className="bg-white text-green-700 px-10 py-4 rounded-2xl font-black uppercase text-sm shadow-2xl hover:bg-gray-100 transition tracking-widest">Shop Now</Link>
          ...
          <div className="flex-1 hidden md:block">
            <div className="relative group">
              <div className="absolute inset-0 bg-green-400 blur-[60px] opacity-20 group-hover:opacity-40 transition duration-700"></div>
              <img 
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          {categories.map((cat) => (
            ...
            >
              ...
              <span className="font-black text-[10px] uppercase tracking-[0.2em] text-gray-400 group-hover:text-green-600 transition">{cat.name}</span>
            </Link>
          ))}
        </div>
      </section>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <div className="lg:col-span-4 space-y-8">
              <h2 className="text-5xl font-black text-white tracking-tighter uppercase leading-[0.85]">
                Wall of <br/><span className="text-green-500">Trust</span>
              </h2>
              ...
              <div className="flex p-1.5 bg-white/5 backdrop-blur-md rounded-[2rem] w-fit border border-white/5">
                <button 
                  ...
                  className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${activeReviewTab === 'top' ? 'bg-green-600 text-white shadow-xl shadow-green-900/40' : 'text-gray-400 hover:text-white'}`}
                >
                  Top Rated
                </button>
                <button 
                  onClick={() => setActiveReviewTab('recent')}
                  className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${activeReviewTab === 'recent' ? 'bg-green-600 text-white shadow-xl shadow-green-900/40' : 'text-gray-400 hover:text-white'}`}
                >
                  Recent
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {displayedReviews.map((rev, idx) => (
                  ...
                  >
                    ...
                    <p className="text-white text-xl font-medium leading-relaxed italic group-hover:text-green-100 transition duration-500 line-clamp-3">"{rev.comment}"</p>
                    <div className="flex items-center gap-4 pt-8 border-t border-white/5 mt-auto">
                      <div className="w-12 h-12 bg-green-600 rounded-2xl flex items-center justify-center font-black text-xs text-white uppercase shadow-lg shadow-green-900/50 group-hover:scale-110 transition">{rev.userName.charAt(0)}</div>
                      <div className="min-w-0">
                        <p className="text-sm font-black text-white uppercase tracking-tighter truncate">{rev.userName}</p>
                        <Link to={`/product/${rev.productId}`} className="text-[10px] text-green-400 hover:text-green-300 font-bold tracking-tight truncate block mt-0.5 transition">Item: {rev.productName}</Link>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div key={product.id} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-50 overflow-hidden group hover:shadow-2xl hover:-translate-y-2 transition-all duration-700">
              <div className="h-64 overflow-hidden relative">
                ...
                <div className="absolute top-6 left-6">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase shadow-xl ${product.productType === 'platform_sell' ? 'bg-green-600 text-white' : 'bg-slate-950 text-white'}`}>
                    {product.productType === 'platform_sell' ? 'E-Shop' : 'Direct'}
                  </span>
                </div>
              </div>
              <div className="p-8 space-y-5">
                <h3 className="font-black text-slate-800 text-xl line-clamp-1 group-hover:text-green-600 transition">{product.name}</h3>
                <div className="flex items-end justify-between">
                  <p className="text-3xl font-black text-slate-900 tracking-tighter">₹{product.price}</p>
                  <span className="text-[10px] font-black text-green-500 bg-green-50 px-3 py-1 rounded-full uppercase tracking-widest">{product.condition}</span>
                </div>
                <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                  ...
                  <Link to={`/product/${product.id}`} className="bg-slate-950 text-white p-3 rounded-2xl hover:bg-green-600 transition shadow-lg">

  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="w-full flex items-center justify-between bg-gray-50 border-2 border-transparent hover:border-gray-200 focus:border-[#16a34a] rounded-2xl px-4 py-3 transition-all text-left"
      >
        ...
        <div className="absolute z-50 w-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-3 border-b border-gray-50">
            <input
              autoFocus
              type="text"
              className="w-full bg-gray-50 border-none rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 ring-[#16a34a]/20"
              placeholder="Search..."
              value={search}
          ...
          <div className="max-h-60 overflow-y-auto">
            <button
              ...
              className={`w-full text-left px-4 py-3 text-sm font-bold hover:bg-green-50 transition-colors ${selected === 'All' ? 'text-green-600 bg-green-50/50' : 'text-slate-700'}`}
            >
              All {label}s
            </button>
            {filteredOptions.map((opt) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-sm font-bold hover:bg-green-50 transition-colors ${selected === opt ? 'text-green-600 bg-green-50/50' : 'text-slate-700'}`}
              >
                {opt}
              </button>
            ))}
        ...
        <aside className="w-full lg:w-80 space-y-6 bg-white p-8 rounded-[2.5rem] border shadow-sm h-fit lg:sticky lg:top-24">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-black text-xl text-slate-800 uppercase tracking-tighter flex items-center gap-2">
              <svg className="w-6 h-6 text-[#16a34a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
              Smart Filters
            </h3>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-gray-50 border-2 border-transparent focus:border-[#16a34a] focus:bg-white rounded-2xl pl-12 pr-4 py-3 outline-none transition-all font-bold placeholder:text-gray-300"
                />
                <svg className="w-5 h-5 absolute left-4 top-3.5 text-gray-400 group-focus-within:text-[#16a34a] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
              </div>
            </div>

            {/* Pincode & Location */}
            <div className="p-5 bg-green-50/50 rounded-3xl space-y-4 border border-green-50">
              <div className="space-y-2">
                <label className="block text-xs font-black text-[#16a34a] uppercase tracking-widest ml-1">♻️ Eco-Friendly Reach</label>
                <input 
                  type="text" 
                  value={pincodeFilter}
                  ...
                  maxLength={6}
                  className="w-full bg-white border-none rounded-xl px-4 py-2 text-sm font-bold shadow-sm focus:ring-2 ring-green-500 outline-none transition-all"
                />
              </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-sm font-black text-[#16a34a] bg-green-50 px-3 py-1 rounded-full tracking-tighter">₹{priceRange}</span>
              </div>
              <input 
                type="range" 
                min="0" 
            ...
            <div className="grid grid-cols-2 gap-3">
               <div className="space-y-2">
                 ...
                 <button 
                  ...
                  className={`w-full py-2.5 rounded-xl text-[10px] font-black uppercase transition-all border-2 ${selectedType === ProductType.PLATFORM_SELL ? 'bg-[#16a34a] text-white border-[#16a34a] shadow-lg shadow-green-200' : 'bg-gray-50 text-gray-400 border-transparent hover:border-gray-100'}`}
                 >
                   E-Shop
                 </button>
               </div>
               <div className="space-y-2">
                 ...
                 <div className="grid grid-cols-2 gap-2">
                   <button 
                    ...
                    className={`py-2.5 rounded-xl text-[9px] font-black uppercase transition-all border-2 ${selectedCondition === Condition.NEW ? 'bg-green-600 text-white border-green-600 shadow-lg shadow-green-200' : 'bg-gray-50 text-gray-400 border-transparent hover:border-gray-100'}`}
                   >
                     🆕 Brand New
                   </button>
                   <button 
                    onClick={() => setSelectedCondition(selectedCondition === Condition.USED ? 'All' : Condition.USED)}
                    className={`py-2.5 rounded-xl text-[9px] font-black uppercase transition-all border-2 ${selectedCondition === Condition.USED ? 'bg-[#16a34a] text-white border-[#16a34a] shadow-lg shadow-green-200' : 'bg-gray-50 text-gray-400 border-transparent hover:border-gray-100'}`}
                   >
                     ♻️ Second-Hand
                   </button>
                 </div>
          ...
          <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="px-3 py-1 bg-green-600 text-white text-[10px] font-black rounded-full shadow-lg shadow-green-100">{filteredProducts.length} Items</div>
                    </div>
            ...
            <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-2xl border shadow-sm self-start md:self-auto group">
               ...
               <select 
                 ...
                 className="bg-transparent text-sm font-black text-green-600 outline-none cursor-pointer pr-2"
               >
          ...
          {filteredProducts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
              {filteredProducts.map(product => {
                ...
                return (
                  <div key={product.id} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-2xl hover:shadow-green-500/5 hover:-translate-y-1 transition-all duration-500">
                    <div className="h-64 overflow-hidden relative">
                      ...
                      <div className="absolute top-5 left-5 flex flex-col gap-2">
                         <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase text-white shadow-xl ${product.productType === ProductType.PLATFORM_SELL ? 'bg-green-600' : 'bg-slate-900'}`}>
                    ...
                    <div className="p-8">
                      <div className="flex justify-between items-start mb-4">
                         <span className="text-[10px] font-black text-green-600 bg-green-50/50 px-4 py-1.5 rounded-full uppercase tracking-widest">{product.category}</span>
                      ...
                      <h3 className="font-black text-slate-800 text-xl line-clamp-1 mb-4 group-hover:text-green-600 transition-colors duration-300">{product.name}</h3>
                      <div className="flex items-end justify-between gap-4">
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-black text-gray-400 flex items-center gap-2 uppercase tracking-tighter bg-gray-50 w-fit pr-3 py-1 rounded-full">
                            <svg className="w-4 h-4 text-green-500 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            {product.location} • {product.pincode}
                          </p>
                        </div>
                        <Link 
                          to={`/product/${product.id}`}
                          className="bg-green-600 text-white p-4 rounded-2xl hover:bg-slate-900 transition-all duration-300 shadow-xl shadow-green-500/20 active:scale-95 flex items-center justify-center shrink-0"
                        >
              ...
              <div className="w-32 h-32 bg-green-50 text-green-500 rounded-[2.5rem] flex items-center justify-center text-6xl mb-8 animate-bounce">🔍</div>
              ...
              <button 
                ...
                className="px-12 py-5 bg-green-600 text-white rounded-[2rem] font-black text-sm uppercase shadow-2xl shadow-green-500/30 hover:scale-105 active:scale-95 transition-all"
              >
                Reset Everything
              </button>
            </div>

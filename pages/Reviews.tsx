    ...
    <div className="pb-24">
      ...
      <section className="bg-slate-900 text-white py-24 px-4 overflow-hidden relative">
        ...
        <div className="max-w-6xl mx-auto relative z-10 text-center">
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-6 leading-none">
            COMMUNITY <br/><span className="text-blue-400">VOICE</span>
          </h1>
      ...
      <section className="max-w-5xl mx-auto px-4 -mt-16 relative z-20">
        <div className="bg-white rounded-[3rem] shadow-2xl p-8 md:p-16 border border-gray-100 flex flex-col items-center">
          <div className="text-xs font-black uppercase tracking-[0.3em] text-blue-600 mb-10">Top Shared Stories</div>
              ...
              {featuredReviews.map((rev, i) => (
                <div key={i} className="min-w-full px-4 flex flex-col items-center text-center space-y-8">
                  ...
                  <div className="flex flex-col items-center pt-8 border-t border-gray-50 w-full">
                    ...
                    <Link to={`/product/${rev.productId}`} className="text-xs font-bold text-blue-600 hover:underline">Item: {rev.productName}</Link>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex gap-4 mt-12">
            ...
            <div className="flex items-center gap-2">
              {featuredReviews.map((_, i) => (
                <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${currentIndex === i ? 'w-8 bg-blue-600' : 'w-2 bg-gray-200'}`}></div>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 p-1.5 bg-white border rounded-[2rem] shadow-sm">
            ...
            {[5, 4, 3, 2, 1].map(star => (
              <button 
                ...
                className={`flex items-center gap-1.5 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${selectedRating === star ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-gray-400 hover:text-slate-800'}`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredReviews.length > 0 ? filteredReviews.map((rev, i) => (
            <div key={i} className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group">
              ...
              <p className="text-slate-700 font-medium leading-relaxed italic group-hover:text-blue-700 transition">"{rev.comment}"</p>
              <div className="flex items-center gap-4 pt-8 border-t border-gray-50 mt-8">
                <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center font-black text-xs">{rev.userName.charAt(0)}</div>
                <div>
                  <p className="text-xs font-black text-slate-800 uppercase tracking-tighter">{rev.userName}</p>
                  <Link to={`/product/${rev.productId}`} className="text-[10px] font-bold text-gray-400 hover:text-blue-600">RE: {rev.productName}</Link>
                </div>
              </div>
            </div>
          )) : (
      ...
      <section className="max-w-7xl mx-auto px-4 mt-24">
        <div className="bg-blue-600 rounded-[3rem] p-12 md:p-24 text-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <p className="text-blue-100 max-w-lg mx-auto font-medium">Have you bought or sold something on Stumarto? Share your experience and help the community grow.</p>
            <Link to="/marketplace" className="inline-block bg-white text-blue-600 px-12 py-5 rounded-[2rem] font-black uppercase text-sm shadow-2xl shadow-blue-900/40 hover:scale-105 transition-all">Start Reviewing</Link>
          </div>
        </div>
      </section>
    </div>

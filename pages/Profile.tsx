...
const Profile: React.FC<ProfileProps> = ({ user, onUpdate, products, onLogout }) => {
  ...
  if (!user) {
    return (
      <div className="max-w-4xl mx-auto py-40 text-center">
        ...
        <button onClick={() => navigate('/login')} className="px-10 py-4 bg-[#2563eb] text-white rounded-2xl font-black uppercase tracking-widest text-xs">Login Now</button>
      </div>
    );
  }
  ...
  const handleLogoutClick = () => {
    if (window.confirm("Are you sure you want to sign out from Stumarto?")) {
      onLogout();
      navigate('/');
    }
  };
      ...
      <div className="flex flex-col lg:flex-row gap-16">
         ...
         <div className="w-full lg:w-96 space-y-8">
            <div className="bg-white rounded-[3rem] border p-12 text-center shadow-xl shadow-slate-100 relative overflow-hidden">
               <div className="absolute top-0 inset-x-0 h-32 bg-[#2563eb]"></div>
               <div className="relative pt-12">
                  <div className="w-32 h-32 bg-white rounded-[2.5rem] border-4 border-white shadow-xl flex items-center justify-center text-5xl font-black text-[#2563eb] mx-auto mb-8">
                    {user.name.charAt(0)}
                  </div>
                  <h2 className="text-3xl font-black text-slate-800 mb-2 uppercase tracking-tighter">{user.name}</h2>
                  <div className="flex items-center justify-center gap-2 mb-10">
                    <span className="text-[10px] font-black text-[#2563eb] uppercase tracking-widest">{user.role} Member</span>
                    <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span>
                  </div>
                  
                  {isEditing ? (
                    <div className="space-y-4 text-left">
                       <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value} as User)} className="w-full border-2 border-gray-100 rounded-2xl p-4 bg-gray-50 outline-none focus:bg-white focus:border-[#2563eb] transition" placeholder="Full Name" />
                       <input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value} as User)} className="w-full border-2 border-gray-100 rounded-2xl p-4 bg-gray-50 outline-none focus:bg-white focus:border-[#2563eb] transition" placeholder="Phone Number" />
                       <div className="flex gap-3 pt-4">
                          <button onClick={handleSave} className="flex-1 py-4 bg-[#2563eb] text-white rounded-xl font-black text-[10px] uppercase tracking-widest">Save</button>
                    ...
                    <div className="space-y-4">
                       <p className="text-gray-500 font-medium leading-relaxed mb-6">{user.bio || 'Building a sustainable school community on Stumarto.'}</p>
                       <div className="space-y-4 pt-8 border-t border-gray-50 text-left mb-10">
                          <div className="flex items-center gap-4 text-sm font-bold text-slate-600">
                             <div className="w-10 h-10 bg-blue-50 text-[#2563eb] rounded-xl flex items-center justify-center text-lg">✉️</div>
                             <span className="truncate">{user.email}</span>
                          </div>
                          <div className="flex items-center gap-4 text-sm font-bold text-slate-600">
                             <div className="w-10 h-10 bg-blue-50 text-[#2563eb] rounded-xl flex items-center justify-center text-lg">📍</div>
                       ...
                       <div className="space-y-3">
                          <button onClick={() => setIsEditing(true)} className="w-full py-5 bg-[#0f172a] text-white rounded-[2rem] font-black text-[10px] uppercase hover:bg-[#2563eb] transition tracking-widest shadow-xl">Update Identity</button>
                          <button 
                            onClick={handleLogoutClick} 
                            className="w-full py-5 bg-gray-50 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-[2rem] font-black text-[10px] uppercase transition tracking-widest flex items-center justify-center gap-3"
                          >
            ...
            {user.role === 'seller' && (
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border shadow-sm text-center">
                     ...
                     <p className="text-2xl font-black text-[#2563eb]">{myOrders.length + 12}</p>
                  </div>
               </div>
            )}
         </div>
         ...
         <div className="flex-1 space-y-12">
            ...
            {user.role === 'seller' && (
               <div className="bg-white rounded-[3.5rem] border p-12 space-y-10 shadow-sm relative">
                  <div className="flex items-center justify-between border-b border-gray-50 pb-8">
                     <h2 className="text-sm font-black text-[#2563eb] uppercase tracking-[0.2em]">Payout Credentials</h2>
                  ...
                  <div className="pt-4 flex justify-end gap-4">
                     {isEditingBank ? (
                        <>
                           <button onClick={handleSaveBank} className="px-10 py-4 bg-[#2563eb] text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-100 transition">Confirm Changes</button>
                        ...
                        <button onClick={() => setIsEditingBank(true)} className="px-10 py-4 bg-gray-50 text-slate-400 hover:text-[#2563eb] rounded-full font-black text-[10px] uppercase tracking-widest transition border border-transparent hover:border-blue-100">Update Bank Account</button>
                     )}
                  </div>
               </div>
            )}
            ...
            <div className="space-y-12">
               <section className="space-y-8">
                  <h3 className="text-xl font-black text-[#2563eb] uppercase tracking-[0.3em] flex items-center gap-4">
                       ...
                       {myOrders.map(order => (
                          <div key={order.id} className="bg-white border border-gray-100 rounded-[3rem] p-8 flex flex-col sm:flex-row justify-between items-center hover:border-blue-200 hover:shadow-2xl transition-all duration-500 gap-6">
                             <div className="flex items-center gap-8">
                                <div className="w-20 h-20 bg-blue-50 rounded-[1.5rem] flex items-center justify-center text-4xl">📦</div>
                                <div>
                             ...
                             <div className="flex items-center gap-8 self-end sm:self-center">
                                <div className="text-right">
                                   ...
                                   <span className={`text-[10px] font-black uppercase px-4 py-1 rounded-full mt-2 inline-block ${order.status === 'Delivered' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{order.status}</span>
                                </div>
                                <button 
                                  onClick={() => navigate(`/track/${order.id}`)}
                                  className="px-8 py-3 bg-[#0f172a] text-white rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest hover:bg-[#2563eb] transition-all shadow-xl shadow-slate-100"
                                >
                                  Track
                                </button>
                             </div>
                       ...
                       {myProducts.slice(0, 4).map(p => (
                          <div key={p.id} className="bg-white border border-gray-100 rounded-[3rem] p-6 flex items-center gap-6 hover:shadow-2xl transition-all duration-500 group">
                             ...
                             <div className="min-w-0 flex-1">
                                ...
                                <p className="text-2xl font-black text-[#2563eb] tracking-tighter">₹{p.price}</p>
                             </div>
                             <Link to={`/product/${p.id}`} className="p-4 bg-gray-50 text-slate-400 rounded-2xl group-hover:bg-[#2563eb] group-hover:text-white transition-all">
                               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"/></svg>
                             </Link>
                          </div>
                       ))}
                    </div>
                    <button onClick={() => navigate('/seller-dashboard')} className="w-full py-6 bg-[#0f172a] text-white rounded-[2.5rem] font-black text-[10px] uppercase tracking-widest hover:bg-[#2563eb] transition-all shadow-2xl">Manage All Listings</button>
                 </section>
               )}
            </div>
         </div>

    ...
    if (token) {
      ...
      (async () => {
        try {
          ...
          console.error('Could not refresh user from token', err);
        }
        ...
        <header className="bg-white sticky top-0 z-50 py-4">
          <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-10">
              ...
              <Link to="/cart" className="relative text-[#1e293b] hover:text-[#16a34a] transition-colors">
                <svg className="w-[22px] h-[22px]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                </svg>
                {cart.length > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-[#16a34a] text-white text-[8px] rounded-full h-4 w-4 flex items-center justify-center font-black">
              ...
              {user ? (
                <Link to="/profile" className="flex items-center gap-2 bg-[#0f172a] text-white pl-1.5 pr-4 py-1.5 rounded-full hover:bg-[#16a34a] transition-all shadow-lg group">
                ...
                <Link to="/login" className="bg-[#0f172a] text-white px-8 py-2.5 rounded-full font-black text-[11px] uppercase tracking-[0.1em] hover:bg-[#16a34a] transition-all shadow-md shadow-slate-200">Login</Link>
              )}
            </nav>
          </div>
